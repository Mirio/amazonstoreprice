...
class TestAmazonStorePrice(TestCase):
    def setUp(self):
        ...
        self.urltest = ("https://www.amazon.it/gp/product/B01LPSG05O/"
                        "ref=s9_simh_gw_p74_d0_i6"
    ...
    def test_geturl(self):
        self.assertEqual(self.amazonstoreprice.getpage(
            self.amazonstoreprice.normalizeurl(self.urltest)).find(
            id="productTitle").contents[0].replace("  ", "").replace("\n", ""),
            "Alla ricerca di Dory")

    def test_getprice(self):
        self.assertIsInstance(self.amazonstoreprice.getprice(
                              self.urltest,retry_ontemp=True), float)
